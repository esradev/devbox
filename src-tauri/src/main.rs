...
fn get_sites_file() -> Result<std::path::PathBuf, String> {
...
#[tauri::command]
fn delete_site(site_id: String) -> Result<(), String> {
    let sites_file = get_sites_file()?;
    let mut sites: Vec<SiteData> = if sites_file.exists() {
        let content = std::fs::read_to_string(&sites_file).map_err(|e| e.to_string())?;
        serde_json::from_str(&content).unwrap_or_default()
...
fn main() {
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![save_site, load_sites, delete_site])
